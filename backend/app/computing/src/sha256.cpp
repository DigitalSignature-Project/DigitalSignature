...
uint32_t ch(uint32_t e, uint32_t f, uint32_t g) {
    // Choose: selects bits from f or g depending on e
    return (e & f) ^ (~e & g);
}

uint32_t maj(uint32_t a, uint32_t b, uint32_t c) {
    // Majority: bit is 1 if at least two inputs have bit 1
    return (a & b) ^ (a & c) ^ (b & c);
}
...
// ------------------------------------------------------------
// Parsing 512‑bit block into 16 words
// ------------------------------------------------------------
...
std::vector<uint8_t> sha256(const std::vector<uint8_t>& message) {
    ...
    // Convert H to big‑endian byte array
