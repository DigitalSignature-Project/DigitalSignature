...
@router.get("/generate_rsa_signature", response_model=RsaSignatureResponse)
async def generate_rsa_signature(data: RsaSignature) -> RsaSignatureResponse:
    file_content = data.file_content
    login = data.login
    password = data.password

    signature = create_rsa_signature(file_content, login, password)
...
async def verify_rsa_sign(data: VerifyRsaSignature) -> VerifyRsaSignatureResponse:
    ...
    result = verify_rsa_signature(file_content, signature, login)
