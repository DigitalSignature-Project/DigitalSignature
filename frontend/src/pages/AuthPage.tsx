...
type ViewMode = 'LOGIN' | 'LOGIN_PASSPHRASE' | 'REGISTER_1' | 'REGISTER_2' | 'FORGOT_PASSWORD';
...
const AuthPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ 
    login: '', repeatLogin: '', password: '', repeatPassword: '', keyPassphrase: '', repeatKeyPassphrase: ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (viewMode === 'LOGIN') {
      // Placeholder for backend logic: e.g., const response = await api.login(...)
      const response = true; 
        ...
        <div className="p-10 flex-1 flex flex-col"> 
          {viewMode === 'FORGOT_PASSWORD' ? (
            <div className="text-center space-y-6 py-4 flex-1 flex flex-col justify-center">
              ...
              <p className="text-slate-500 text-sm">This feature is not yet available.</p>
              <button 
              ...
              {viewMode === 'LOGIN' && (
                <>
                  <div className="relative">
                    ...
                    <input
                      name="login" type="text" required placeholder="Username"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={() => changeView('FORGOT_PASSWORD')} className="text-sm font-medium text-slate-400 hover:text-[#0f172a] transition-colors underline underline-offset-4">
                      Forgot password?
                    </button>
                  </div>
                </>
              ...
              {viewMode === 'LOGIN_PASSPHRASE' && (
                <>
                  <p className="text-sm text-slate-600 font-medium text-center mb-2">
                    Enter your private key passphrase to unlock your identity on this device.
                  </p>
                  <div className="relative">
                    <Key className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                    <input
                      name="keyPassphrase" type="password" required placeholder="Private Key Passphrase"
                      className="w-full pl-12 pr-4 py-3.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 outline-none transition-all bg-slate-50/50"
                      value={formData.keyPassphrase} onChange={handleChange}
                    />
                  </div>
                </>
              )}
              ...
              {viewMode === 'REGISTER_1' && (
                <>
                  <div className="space-y-3">
                    <input
                      name="login" type="text" required placeholder="New Username"
                      className="w-full px-4 py-3 border border-slate-200 rounded-xl outline-none bg-slate-50/50"
                      value={formData.login} onChange={handleChange}
                    />
                    <input
                      name="repeatLogin" type="text" required placeholder="Repeat Username"
                      className="w-full px-4 py-3 border border-slate-200 rounded-xl outline-none bg-slate-50/50"
                      value={formData.repeatLogin} onChange={handleChange}
                    />
                    <input
                      name="password" type="password" required placeholder="New Password"
                      className="w-full px-4 py-3 border border-slate-200 rounded-xl outline-none bg-slate-50/50"
                      value={formData.password} onChange={handleChange}
                    />
                    <input
                      name="repeatPassword" type="password" required placeholder="Repeat Password"
                      className="w-full px-4 py-3 border border-slate-200 rounded-xl outline-none bg-slate-50/50"
              ...
              {viewMode === 'REGISTER_2' && (
                <>
                  <p className="text-sm text-slate-600 font-medium text-center mb-4">
                    Set a passphrase to encrypt your private key locally. This cannot be recovered.
                  </p>
                  <div className="space-y-3">
                    <input
                      name="keyPassphrase" type="password" required placeholder="Key Passphrase"
                      className="w-full px-4 py-3 border border-slate-200 rounded-xl outline-none bg-slate-50/50"
                      value={formData.keyPassphrase} onChange={handleChange}
                    />
                    <input
                      name="repeatKeyPassphrase" type="password" required placeholder="Repeat Passphrase"
                      className="w-full px-4 py-3 border border-slate-200 rounded-xl outline-none bg-slate-50/50"
                      value={formData.repeatKeyPassphrase} onChange={handleChange}
                    />
                  </div>
                </>
              )}
            ...
            <div className="mt-auto pt-6 border-t border-slate-100 text-center">
              {viewMode === 'LOGIN' && (
                <p className="text-sm text-slate-500">
                  New here?
                  <button type="button" onClick={() => changeView('REGISTER_1')} className="ml-2 font-bold text-[#0f172a] hover:text-cyan-600 transition-colors">
                    Create Account
                  </button>
                </p>
              )}
              ...
              {viewMode === 'LOGIN_PASSPHRASE' && (
                <button type="button" onClick={() => changeView('FORGOT_PASSWORD')} className="text-sm font-bold text-slate-400 hover:text-red-500 transition-colors">
                  Reset private key
                </button>
              )}

              {(viewMode === 'REGISTER_1' || viewMode === 'REGISTER_2') && (
                <button type="button" onClick={() => changeView('LOGIN')} className="text-sm font-bold text-[#0f172a] hover:text-cyan-600 transition-colors">
                  Back to Login
                </button>
              )}
            </div>
