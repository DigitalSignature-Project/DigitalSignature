...
def setup_project():
    ...
    python_exe_path = (
        ...
        if is_windows
        else "${workspaceFolder}/backend/.venv/bin/python"
    )
...
def build_project():
    ...
    if is_windows:
        clean_env["PATH"] = ";".join(
            [
                p
                for p in clean_env.get("PATH", "").split(";")
    ...
    if vcpkg_exe.exists():
        ...
        print(
            f"{Colors.WARNING}vcpkg executable not found in {vcpkg_root}. Skipping auto-installation.{Colors.ENDC}"
        )
...
def run_project():
    ...
    if not python_exe.exists():
        print_error(
            "Virtual environment not found! Please run 'python manage.py setup' first."
        )
